...
- `#[Inject]` works on a public method, so the container can call a setter: `#[Inject] public function setClock(Clock $clock): void`. Arguments resolve through the same path a constructor's do, so contextual bindings, a nested `#[Inject]` naming an implementation, and defaults all behave identically. Calls run after the constructor and after property injection, in declaration order — observable, so it is fixed rather than left to reflection. A **static or non-public** `#[Inject]` method is refused by name rather than skipped, the way readonly property injection already is: silently ignoring an annotation someone wrote is the worse failure, since nothing says the dependency never arrived. Under `#[Lazy]` the calls sit inside the initializer with the constructor, so laziness still defers them. `ContainerCompiler` refuses a class with injected methods — a `new` expression cannot make the calls — under a new `CompilationSkipReason::InjectedMethod`. This is an escape hatch for code whose constructor is not yours, not an equal alternative to constructor injection; see [best practices](docs/best-practices.md#when-to-reach-for-setter-injection) for when not to reach for it
- `vendor/bin/gacela-container` compiles plans and factories, so the bootstrap script every application was writing by hand — build a container, enumerate classes, call the write methods with a stamp — ships instead of being rewritten. `compile` writes either file or both and says what it wrote; `report` prints `compileReport()` with a `CompilationSkipReason` per refusal, and `--strict` exits non-zero so a build can assert that a class it expects to be compiled actually is. Configuration is a `gacela-container.php` returning your container, which is not a convenience: `ContainerCompiler` refuses a *bound* class, so a container built without your bindings would happily generate a `new` for something the application binds. It also refuses to guess — an unknown or misspelled option is an error rather than an ignored argument, and a discovered set that could not be autoloaded is reported instead of counted as zero work done. No new dependency: the argv parsing is thirty lines, and `psr/container` remains the only runtime requirement. `@internal`, so the CLI is a build tool and not a BC surface. See [performance](docs/performance.md#compiling-from-the-command-line)
- `ClassSource` gives the compile calls somewhere to get their classes from, instead of a `list<class-string>` that nothing produced and every application maintained by hand — where a class added to the application and not to the list silently dropped back to the reflection path with nothing to say so. `ClassSource::fromComposerClassmap()` reads the map Composer already writes for an optimized autoloader; `fromDirectory('src/')` tokenises the tree for dev setups without one, so a file that declares nothing is never included for its side effects; `fromList()` wraps a list you already have. Accepted by `compile()`, `compileReport()`, `writeCompiledCache()` and `writeCompiledFactories()`, and **not** by `warmUp()`, which is the important half: warming *resolves*, so a discovered set goes through a describe-only path that runs no constructors and throws on nothing, where warming a classmap would construct the application and fail on the first class whose scalar nothing supplies. Passing a list still behaves exactly as before, including through `warmUp()`. On `Container` rather than the interfaces, which 1.x promises not to change. See [performance](docs/performance.md#finding-the-classes-to-compile)
- `dependencyGraph()` returns the dependency graph as a tree, where `getDependencyTree()` returns a flat deduplicated list. Despite its name that list is not a tree, and what flattening removes is exactly what a dependency inspector is opened for: how deep something sits, which constructor parameter asked for it, that three parents each pull in the same class, and where a cycle closes. A `DependencyNode` carries all four, plus `render()` for an indented text tree you can `echo` from a breakpoint. A cycle is marked and cut rather than thrown, unlike resolution — inspecting a broken graph is precisely when this is reached for. `getDependencyTree()` is unchanged and now derived from the graph, so the two cannot disagree. See [cookbook](docs/cookbook.md#work-out-why-something-resolves-to-the-wrong-thing)
...
- The benchmark job is required rather than advisory, and every subject is gated rather than five of twenty-three. It was left advisory pending a measurement of the false-positive rate on shared runners; #132 supplied it — the suite reports under ±3.2% and the baseline comes from the merge base — and in the meantime #150 shipped a 12% regression with the job reporting green, which is what an advisory gate is worth. Two thresholds now, both declared on the class so a *new* benchmark is gated by existing and a contributor argues their way out of a gate rather than into one: ±20% on time as a cliff detector, and ±5% on peak memory, which is deterministic and was what half the recent performance work was actually about while nothing gated it. The five subjects `docs/performance.md` quotes figures for carry a tighter ±7% budget, roughly twice the noise floor — the cliff gate catches a collapse like #45, the budget gate catches a drift like #150. Verified in both directions: a deliberate 10% slowdown fails 13 variants and exits non-zero, and an unchanged tree passes cleanly. The comparison table is also posted on the pull request itself, pass or fail, and `composer bench:compare` runs the same A/B locally — warm-up discard included, which is the step every hand-rolled version of it has skipped. See [performance](docs/performance.md#what-is-gated-and-at-what)
...
- `ContainerStats::memoryUsageBytes` is documented as what it is: `memory_get_usage(true)`, the whole PHP process, not the container. Sat among five container-scoped counters it read as "what this container costs", which is not the question it answers — it moves when anything anywhere allocates, and two containers in the same process report the same number. `stats()`'s docblock sold the field on its type ("an int rather than a string needing to be parsed") and said nothing about its meaning. Now stated on the object, on both accessors and in [introspection](docs/services.md#introspection), and pinned by tests so the docs cannot drift from it. The value is unchanged — measuring one container's footprint would mean accounting code on the registration paths to feed a debug field — and the field is renamed `processMemoryBytes` at 2.0, where the name will say what it holds
...
- Dropping a container released it only when the cycle collector next ran, because three of its collaborators held a strong reference back to it and made every container a cycle of nine objects. `createScope()` documents the opposite — "drop the reference and everything it resolved is released" — and a worker creating a scope per request accumulated every scope and everything it resolved until the collector happened to fire, or forever under `gc_disable()`. A thousand cold containers held 1.51mb that way, and around 120kb of it survived an explicit `gc_collect_cycles()`; the same loop retains nothing now. The back-pointer is one `WeakReference` shared by the collaborators that need it, so refcounting alone frees a dropped container now. An *uninitialized* lazy object is the deliberate exception: it resolves after `get()` returned, so it keeps its container alive until first touch. See [scopes](docs/scopes.md#disposal-is-dropping-the-reference)
