...
- Warm resolution memoizes a per-class constructor, taking the resolution path off the graph entirely once a class has been proven statically buildable. A warm four-level chain goes **1.475μs → 0.656μs (−54%)**, past the figure generated factories achieve with the same reflection cached — the remaining cost really was dispatch. `make()` without runtime arguments is −53%. The cost is on the cold path and is real: building the closures is work a container that is thrown away never amortises, so cold subjects are **+7 to +11%** and a class that refuses a builder is +5 to +12%. Long-lived containers halve their resolution; per-request containers pay about 8%. Safety is structural rather than a list of memos to invalidate: everything consulted is either a fact about a class definition, which cannot change within a process, or an array checked on every call — a scope, any contextual binding, any `lazy()` registration all take the mechanism out of play with nothing to invalidate. Bindings get an explicit drop, one per registration method. Every wrong answer falls back to the resolution path rather than producing a wrong object
...
- `give(null)` threw instead of injecting `null`. Every other value already worked — arrays, plain strings, bools, floats — but the lookup used `isset()`, which reports a bound `null` as absent, so the one binding that says "this consumer gets nothing" was the one that could not be expressed. The workaround was `give(static fn () => null)`, which reads as though the laziness mattered when only the value did. For a **type** need `null` is now refused with a message saying why: not binding a type already means "nothing is bound", so binding one to `null` could only be a mistake, and a silent one. See [bindings](docs/bindings.md#named-scalar-contextual-bindings)
...
- `load()` and `loadFile()` take an optional listener, called with each id as it is registered in definition order — the only reliable answer to "what did this source register", since reading the ids back off the container catches `bind()` and `set()` entries but misses aliases, which live in a third registry. An additive parameter: existing calls are untouched, and `FullContainerInterface` is unchanged, so no implementor is affected. See [definitions](docs/definitions.md#knowing-what-a-source-registered)
- `Container::validate()` proves a set of classes resolves *without resolving them*, so a broken wiring fails a deploy instead of a request — the one thing an autowiring container genuinely could not tell you ahead of time. It reports what is decidable: a class that does not exist, an interface or abstract with nothing bound to it, a parameter nothing can supply, a cycle — each with the chain that reached it, because "which of my entry points does this break" is the question a build asks. A cycle is listed rather than thrown, so one run reports every problem. Nothing is constructed: classes are described, and whether an id is satisfiable is answered by `has()` on your own container, so bindings, aliases, stored instances and parent scopes all count and this cannot drift from resolution by re-deriving it. It does not guess what a closure binding returns — only running that settles it, and guessing would make this the second resolver the compiled file is careful not to be. `vendor/bin/gacela-container validate` exits non-zero on any problem. On `Container` rather than either interface, which 1.x promises not to change. See [performance](docs/performance.md#proving-it-resolves-before-it-runs)
