...
- A `when()` call made after `createScope()` was silently invisible to that scope. The scope keeps a copy of its parent's contextual bindings — a chain walk would land on the hot path of every nested parameter to serve a map that is configuration — so a binding registered later never reached it: the scope kept resolving the unbound implementation, with nothing to say so. A wrong object injected in production with a green test suite, and the same shape as the resolver bug fixed in 1.3. The copy stays, and the parent now hands late registrations down to the scopes it created, and to theirs; a scope that registered the same contextual binding for itself keeps its own, the way shadowing works everywhere else. The same treatment for the two maps with the same copy-at-creation semantics: `useCompiledFactories()` and `lazy()`, where a missed entry costs an optimisation rather than a wrong result. The handles a parent keeps on its scopes are weak, so a dropped scope is collected exactly as before — a scope is still usable as a request lifetime — and the list is swept on a growing threshold, so `createScope()` stays constant-time (1.006μs → 1.291μs; the resolution path is untouched, and `benchCreateScope` now tracks it). Ordering between `when()` and `createScope()` no longer matters. See [scopes](docs/scopes.md#what-does-not-fall-through)

### Added

- Keyed tags: `tag(['email' => EmailHandler::class, 'sms' => SmsHandler::class], 'handlers')` and `taggedByKey('handlers', 'sms')`, which is what a command bus, a router or a strategy map actually asks a tag for — *the* handler for a key, not all of them. Consumers were building this themselves as a `key => class-string` map that caches resolved instances in its own array, a second resolution cache in front of a container that already has one; here the id map is the only new thing and the instance comes from the container's own cache, so a singleton still lives in exactly one place. Registration stays one method: a map keys its entries, a list or a single id appends and dedupes as before, and both kinds live in one tag — `tagged()` yields keyed entries under their key and unkeyed ones under their position, so existing loops are untouched. Only the entry asked for is built. Registering a key again replaces it, making per-environment layering a matter of registration order, and a scope can shadow an inherited key for itself the way it shadows a binding. An unknown key throws a `ContainerException` naming the keys the tag does have, with a did-you-mean for near misses, rather than returning `null` — `taggedKeys()` answers when the key is genuinely optional. `taggedByKey()` and `taggedKeys()` live on `Container` rather than `ContainerInterface`, which 1.x promises not to extend. See [tags](docs/bindings.md#keyed-tags)

- `PlanCache`, one constructor-plan cache shared by containers that are not related. Plans were cached per container instance, and `createScope()` only shares them down the parent axis, so sibling roots — one container per module, which is where a modular application ends up — each re-planned whatever the modules had in common. `new Container($bindings, [], [], $plans)` on each of them makes the first to touch a class plan it for the rest: ten containers resolving the same four-level chain go from 55.315μs / 82.61mb to 37.067μs / 45.79mb. What travels is reflection output and nothing else — a plan records what a constructor asks for, never how a container satisfied it — so bindings, contextual bindings, aliases, tags, singletons, stored instances, `lazy()` registrations and compiled factories stay private to the container they were registered on, and two containers sharing a cache still resolve the same class differently when their bindings differ. It takes an optional compiled-plans array, so several containers share one read of a compiled cache file, and a plan already in the cache wins over a compiled one for the same class: the first was reflected in this process, the second came off disk. `count()` and `classes()` report what it holds. The internal `PlanRegistry` stays internal — `PlanCache` is the handle, so the registry keeps its freedom to change. See [performance](docs/performance.md#one-plan-cache-for-several-containers)

- `Container::resetStaticCaches()`, clearing the reflection caches that outlive every container. Four of them are `static` — property plans, `#[Lazy]` on a class, proven-instantiable classes, has-`#[Inject]`-properties — so dropping a container never touched them and nothing outside the package could reach them. That is free while the set of loadable classes is fixed, and wrong when it is not: code generation, a cache-warm command, a worker that re-bootstraps, a suite that declares classes as it goes. Each cache is now documented by *lifetime* — keyed on a class's shape, so cleared; or on the PHP binary, so recomputed identically — because whether a cache needs invalidating should not have to be decided by reading it. Only positives are ever stored, so a class that was not loadable when first asked about is never remembered as missing, and the reset touches nothing a container was asked to keep: singletons, instances and bindings are instance state and survive. A test enumerates every `static` property under `src/` and asserts it is back at its declared default afterwards, so a new one cannot be added without being wired in — the guard a consumer cannot write for its vendors. It lives on `Container` rather than `ContainerInterface`, which 1.x promises not to extend. See [performance](docs/performance.md#process-global-caches)

- A validity fingerprint on every compiled entry, so a stale cache can no longer be served as if current. `writeCompiledCache()` and `writeCompiledFactories()` stamp each entry with the file its class was declared in (path, mtime, size), and `Container::loadCompiledCache()` / the new `Container::loadCompiledFactories()` drop the entries whose file has changed: a stale entry behaves exactly like a missing one, the class falls back to reflection, and correctness never depends on the cache being up to date. A deleted file counts as changed; a class with no file of its own — an internal one — cannot be invalidated by an edit and is kept. The file rather than the constructor signature, because verifying a signature means reflecting the class, which is the work the cache exists to avoid. Where one `stat` per class costs more than the reflection it saves, both write and load take an optional build stamp — a deploy id, a commit sha: matching stamps take the file whole with no per-entry check, a mismatch discards it whole, and no stamp on either side falls back to per-entry checks. The cache file grew an envelope (a format marker, the build stamp, the stamps, then the entries) and is still a plain `return [...]` that opcache maps; read it with `loadCompiledCache()`/`loadCompiledFactories()` rather than `require`, which sees the envelope instead of the map. A file written by another version is refused with a `ContainerException` instead of being half-read. `ContainerInterface::writeCompiledCache()` is deliberately left alone — widening an interface 1.x promises not to extend would break every implementation of it — so the build-stamp argument lives on `Container` only. See [performance](docs/performance.md#staleness)
